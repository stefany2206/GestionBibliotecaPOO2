...
public class GestionUsuariosForm extends JFrame {
    ...
    public GestionUsuariosForm() {
        ...
        // Columnas solo para estudiantes
        modelo = new DefaultTableModel();
        modelo.setColumnIdentifiers(new String[]{
                "Nombre", "ID", "Correo", "Teléfono", "Dirección",
                "Carrera", "Código Institucional", "Año Ingreso", "Clave"
        });
        ...
        tableUsuarios.getSelectionModel().addListSelectionListener(e -> {
            ...
            if (fila >= 0) {
                ...
                txtCarrera.setText(modelo.getValueAt(fila, 5).toString());
                txtCodigoInstitucional.setText(modelo.getValueAt(fila, 6).toString());
                txtAnioIngreso.setText(modelo.getValueAt(fila, 7).toString());
    ...
    private void agregar() {
        ...
        String codigoInst = txtCodigoInstitucional.getText().trim();
        ...
        if (nombre.isEmpty() || id.isEmpty() || correo.isEmpty() || telefono.isEmpty() ||
                ...
                codigoInst.isEmpty() || anioIngreso.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Todos los campos son obligatorios.");
            return;
        }

        String sql = "INSERT INTO usuarios " +
                "(id, nombre, correo, telefono, direccion, rol, clave, carrera, codigo_institucional, anio_ingreso) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        try (Connection conn = ConexionBD.conectar();
            ...
            ps.setString(6, ROL_ESTUDIANTE); // forzado
            ps.setString(7, clave);
            ps.setString(8, carrera);
            ps.setString(9, codigoInst);
            ps.setString(10, anioIngreso);

            ps.executeUpdate();

            modelo.addRow(new String[]{
                    nombre, id, correo, telefono, direccion,
                    carrera, codigoInst, anioIngreso, clave
            });
            limpiarCampos();
    ...
    private void actualizar() {
        ...
        String sql = "UPDATE usuarios SET nombre=?, correo=?, telefono=?, direccion=?, " +
                "rol=?, clave=?, carrera=?, codigo_institucional=?, anio_ingreso=? WHERE id=? AND rol=?";
        ...
        try (Connection conn = ConexionBD.conectar();
            ...
            ps.setString(5, ROL_ESTUDIANTE); // forzado
            ps.setString(6, clave);
            ps.setString(7, carrera);
            ps.setString(8, codigoInst);
            ps.setString(9, anioIngreso);
            ps.setString(10, id);
            ps.setString(11, ROL_ESTUDIANTE);
            ...
            modelo.setValueAt(carrera, fila, 5);
            modelo.setValueAt(codigoInst, fila, 6);
            modelo.setValueAt(anioIngreso, fila, 7);
    ...
    private void buscar() {
        ...
        JOptionPane.showMessageDialog(this, "ID no encontrado (solo estudiantes).");
    }
    ...
    private void cargarEstudiantes() {
        ...
        String sql = "SELECT id, nombre, correo, telefono, direccion, clave, carrera, codigo_institucional, anio_ingreso " +
                "FROM usuarios WHERE rol = ?";

        try (Connection conn = ConexionBD.conectar();
             PreparedStatement ps = conn.prepareStatement(sql)) {

            ps.setString(1, ROL_ESTUDIANTE);
